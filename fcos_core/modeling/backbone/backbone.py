...
def build_resnet_fpn_p3p7_backbone(cfg):
    ...
    fpn = fpn_module.FPN(
        ...
        conv1xkif=cfg.MODEL.BACKBONE.CONV1XK
    )
    model = nn.Sequential(OrderedDict([("body", body), ("fpn", fpn)]))
    model.out_channels = out_channels
    return model
...
@registry.BACKBONES.register("R-101-FPN-DDTNET")
...
@registry.BACKBONES.register("R-101-SSD-Orig")
def build_ssd_backbone(cfg):
    body = ssd.resnetssd101_Orig(cfg)
    model = nn.Sequential(OrderedDict([("body", body)]))
    model.out_channels = cfg.MODEL.SSD.BACKBONE_OUT_CHANNELS
    return model


@registry.BACKBONES.register("R-101-SSD-1xk")
def build_ssd_backbone(cfg):
    body = ssd.ResNetSSD_1xk(cfg)
    model = nn.Sequential(OrderedDict([("body", body)]))
    model.out_channels = cfg.MODEL.SSD.BACKBONE_OUT_CHANNELS
    return model
